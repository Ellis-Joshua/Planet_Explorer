...
public class ClickableUI extends ClickableObject {
	...
	public void setup_ui(Game game,Context context){
		...
		button_up= new ClickableObject (frameDom,button_size, buttonAt, 2,1, ClickableObject.RECTANGLE);// creat the button
		...
		button_down= new ClickableObject (frameDom,button_size, buttonAt, 2,1, ClickableObject.RECTANGLE);// creat the player object
		...
		button_left= new ClickableObject (frameDom,button_size, buttonAt, 2,1, ClickableObject.RECTANGLE);// creat the player object
		...
		button_right= new ClickableObject (frameDom,button_size, buttonAt, 2,1, ClickableObject.RECTANGLE);// creat the player object
		...
		button_stop= new ClickableObject (frameDom,button_size, buttonAt, 2,1, ClickableObject.RECTANGLE);// creat the player object
		
		full_UI=Bitmap.createBitmap(displaySize.x,displaySize.y,Config.ARGB_8888);
	...
	public void reset_button_click_state(){
		button_up.setFrame(0);
		button_down.setFrame(0);
		button_left.setFrame(0);
		button_right.setFrame(0);
		Update_UI();
	}
	public void onClick(RenderableObject player, Point touch, int speed, boolean touch_down) {
		if (touch_down){
			if (button_up.isClicked(touch)){
				button_up.setFrame(1);
				Update_UI();
			}else if (button_down.isClicked(touch)){
				button_down.setFrame(1);
				Update_UI();
			}else if (button_left.isClicked(touch)){
				button_left.setFrame(1);
				Update_UI();
			}else if (button_right.isClicked(touch)){
				button_right.setFrame(1);
				Update_UI();
			}			
		}else{
